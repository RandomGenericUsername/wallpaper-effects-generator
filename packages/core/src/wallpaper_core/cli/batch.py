...
def _resolve_batch_items(
    ...
    item_groups: list[tuple[str, str, str | None]] = []  # (name, type, subdir_for_single_type)
    if batch_type in ("effects", "all"):
        for name in config.effects:
            item_groups.append((name, "effect", "effects" if batch_type != "all" or not flat else None))
    if batch_type in ("composites", "all"):
        for name in config.composites:
            item_groups.append((name, "composite", "composites" if batch_type != "all" or not flat else None))
    if batch_type in ("presets", "all"):
        for name in config.presets:
            item_groups.append((name, "preset", "presets" if batch_type != "all" or not flat else None))
    ...
    for name, item_type, single_type_subdir in item_groups:
        ...
        if batch_type == "all":
            ...
            if flat:
                ...
                subdir_map = {"effect": "effects", "composite": "composites", "preset": "presets"}
        ...
        if item_type == "effect":
            ...
            if composite_def is not None:
                chain_cmds = _resolve_chain_commands(
                    composite_def.chain, config, input_file, out_path,
                )
                cmd = " && ".join(chain_cmds)
...
def batch_composites(
    ...
    _run_batch(ctx, input_file, output_dir, "composites", parallel, strict, flat, dry_run)
